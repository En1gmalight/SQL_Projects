...
-- HW_7
-- 1.1 Выбираю продукты у которых в названии есть 'max' и пробразовываю их в верхний регистр 
SELECT upper(product_name) AS name_uppercase
FROM products
WHERE product_name LIKE '%max%';
-- 1.2 объединяю имя клиента и его email  в одну строку 
SELECT concat(customer_name, ' (', email, ')') AS customer_info
FROM customers;
-- 1.3 запрос вычисляет длины всех строк, сортирует их по убыванию длины и выводит один результат 
SELECT product_name, LENGTH(product_name) AS name_length
FROM products
ORDER BY LENGTH(product_name) DESC
LIMIT 1;
-- 1.4 возвращает позицию первого вхождения подстроки 'Pro', получаю часть строки после слова Pro 
select product_name, 
locate('Pro', product_name) as position,
right(product_name, length(product_name) - locate('Pro', product_name)+1) AS after_pro
from products
where locate('Pro', product_name) > 0;    
 
-- 2.1 Округление до ближайшего целого числа
 SELECT product_name, round(price) AS rounded_price
 FROM products;
-- 2.2 абсолютное значение разницы между ценой каждого товара и значением 2000
SELECT order_item_id, product_id, price, ABS(price - 2000) AS difference
FROM order_items;

-- 3.1 получаю дату создания заказа и текущую дату 
SELECT order_id, order_date, NOW() AS current_datetime
FROM orders;
-- 3.2 получаю заказы сделанные в текущем году
SELECT order_id, customer_id, order_date, total
FROM orders
WHERE YEAR(order_date) = YEAR(CURDATE());
-- 3.3 получаю заказы с разницей в днях от текущей даты
SELECT order_id, order_date, total, DATEDIFF(CURDATE(), order_date) AS days_since_order
FROM orders;

-- 4.1 запрос с использованием coalesce
SELECT customer_name, COALESCE(phone_number, 'no phone number') AS phone_number
FROM customers;
-- 4.2 запрос с использованием ifnull 
SELECT customer_name, IFNULL(phone_number, 'no phone number') AS phone_number
FROM customers;


-- HW_8
-- 1 выборка данных из одной таблицы с использованием всех агрегатных функций (AVG, COUNT, MIN, MAX, SUM)
SELECT 
...
-- 2.1 в один из запросов добавьте функцию ROUND(), чтобы округлить среднее значение до двух десятичных знаков
SELECT round(AVG(price), 2) AS avg_price_rounded
FROM products;
-- 2.2
SELECT round(AVG(total), 2) AS avg_total_rounded
FROM orders;

-- 3 запрос с группировкой при помощи оператора GROUP BY с использованием HAVING
-- запрос фильтрует города в которых количество клиентов больше одного
SELECT city_name, cities.city_id, COUNT(customer_id) AS customer_count
FROM customers
JOIN cities ON customers.city_id = cities.city_id
GROUP BY city_name, cities.city_id
HAVING COUNT(customer_id) > 1;

-- 4 запрос с группировкой с использованием оператора CASE
-- запрос подсчитывает общую стоимость продуктов в каждой ценовой категории
SELECT 
    CASE 
        WHEN price < 1000 THEN 'Low'
        WHEN price BETWEEN 1000 AND 2500 THEN 'Medium'
        ELSE 'High'
    END AS price_range,
...
-- 5 запрос вычисляет средний рейтинг продуктов. Если у продукта нет отзывов, то вместо среднего рейтинга выводится 'no reviews'
SELECT 
    p.product_name, 
    COALESCE(AVG(r.rating), 'no reviews') AS average_rating
FROM products p
...
-- HW_9 
-- 1 SELECT с подзапросом в SELECT
-- этот запрос выбирает имена клиентов и количество их заказов используя подзапрос для подсчета их заказов 
SELECT 
    customer_name, 
    (SELECT COUNT(*) FROM orders o WHERE o.customer_id = c.customer_id) AS order_count
FROM customers c;

-- 2 SELECT с подзапросом в WHERE
-- этот запрос выбирает продукты, которые были заказаны более чем 5 раз 
SELECT p.product_name, p.price
FROM products p
WHERE (SELECT COUNT(*) FROM order_items oi WHERE oi.product_id = p.product_id) > 5;

-- 3 SELECT с подзапросом в FROM
-- запрос выбирает продукты у которых цена выше средней цены всех продуктов
SELECT p.product_name, p.price
FROM products p
JOIN (
...
-- 4 подзапрос с INSERT
-- этот запрос добавляет новый заказ для клиента, используя подзапрос для получения customer_id
INSERT INTO orders (customer_id, order_date, total) VALUES 
((SELECT customer_id FROM customers WHERE customer_name = 'Customer 5'), '2024-10-25', 1500.00);

-- 5 подзапрос с UPDATE
-- обновление цены продукта iPhone 16 Pro Max 256GB на цену продукта iPhone 16 Pro Max 512GB
UPDATE products
JOIN (
    SELECT price AS new_price
    FROM products
    WHERE product_name = 'iPhone 16 Pro Max 512GB'
) AS temp
ON products.product_name = 'iPhone 16 Pro Max 256GB'
SET products.price = temp.new_price;

-- 6 подзапрос с DELETE 
-- этот запрос удалит заказ у которого сумма минимальная среди всех заказов
DELETE FROM orders
WHERE total = (SELECT MIN(total) FROM (SELECT total FROM orders) AS temp);

-- 7 запрос с использованием EXISTS
-- этот запрос выберет имена клиентов у которых есть заказы 
